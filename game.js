...
var initAudio = function(){
	...
	if (!App.ext.useragent.ie) {
			this.track0 = new App.client._Audio.MultiChannelSound("sounds/sound.ogg", 1);
			this.track1 = new App.client._Audio.MultiChannelSound("sounds/qubodupImpactMeat01.ogg", 1);
			this.track2 = new App.client._Audio.MultiChannelSound("sounds/splash.ogg", 1);
			setTimeout(function () {
...
Game = Object.create({
	...
	init: function () {
		...
		this.imgBuffer[13] = this.graphics.load("B05", "images/FlappyFishB04");
		this.imgBuffer[14] = this.graphics.load("B06", "images/FlappyFishB05");
		this.imgBuffer[6] = this.graphics.load("B04", "images/FFCC");

		this.imgBuffer[7] = this.graphics.load("T0L", "images/FlappyFishT0L");
		this.imgBuffer[8] = this.graphics.load("T0R", "images/FlappyFishT0R");
		this.imgBuffer[9] = this.graphics.load("F0L", "images/FlappyFishOutline");
		this.imgBuffer[10] = this.graphics.load("F03", "images/FlappyFish3");
		this.imgBuffer[11] = this.graphics.load("F04", "images/FlappyFish4");
		this.imgBuffer[12] = this.graphics.load("Stop", "images/FlappyFishStart");
		this.splash = 0;
		this.SplashY = 0;
		this.score = 0;
		...
		for (var i = 255; i > 0; --i) {
			...
			var yy = Math.sin(this.TubeX) * 100 - 100;
			if (i % 3 == 0)
				var yy = Math.cos(this.TubeX) * 100 - 100;
	...
	updateLogic:function(){
		this.vx = this.app.client.setWidth * 0.4;
		this.vy = this.FishY + 5 + Math.cos(this.viewX / 15) * 5 + this.app.client.setHeight * 0.85;

		for (var i = this.tubeArray.length - 1; i >= 0; --i) {
			if ((this.vy + (App.delta*5) > this.tubeArray[i].y) && (this.vy < this.tubeArray[i].y + 400) && (this.vx - (App.delta*5) > -this.TubexX + this.tubeArray[i].x) && (this.vx + (App.delta*5) < -this.TubexX + this.tubeArray[i].x + 50))
				this.hit = true;
		}
	...
	draw: function () {
		//Background
		this.visuals.rect_gradient(this.app.client.setWidth * 0.5, this.app.client.setHeight * 0.5, this.app.client.setWidth, this.app.client.setHeight,1, 1, 1, "#4cc1ca", "#FFFFFF", 0);
		//Clouds
		...
		///Fish
		if (!this.continue)
			this.visuals.image_rotate(this.imgBuffer[this.birdimg], this.vx, this.vy, 1, this.FishSpeed * 8 + 0, 1, 0, 0);
		else
			if (this.vy < this.app.client.setHeight * 0.91)
				this.visuals.image_rotate(this.imgBuffer[this.birdimg], this.vx, this.vy, 1, this.FishAngle * 1.5, 1, 0, 0);
			else {
				this.visuals.image_rotate(this.imgBuffer[this.birdimg],this.vx, Math.sin(this.viewX / 15) * 5 + this.app.client.setHeight * 0.9,
					1, 180, 1, 0, 0);
		...
		for (var i = this.tubeArray.length - 1; i >= 0; --i) {
			if ((-this.TubexX + this.tubeArray[i].x < 400) && (-this.TubexX + this.tubeArray[i].x > -100))
				this.drawTube(-this.TubexX + this.tubeArray[i].x, this.tubeArray[i].y);
			// if ((this.vy + 5 > this.tubeArray[i].y) && (this.vy < this.tubeArray[i].y + 400) && (this.vx + 5 > -this.TubexX + this.tubeArray[i].x) && (this.vx + 5 < -this.TubexX + this.tubeArray[i].x + 50))
			// 	this.hit = true;
		}
		//Water
		...
		//GetReady
		if (!this.continue) {
			this.visuals.image_ext(this.imgBuffer[9], -5 + this.app.client.setWidth * 0.5, 15 + Math.cos(this.viewX / 15) * 5 + this.app.client.setHeight * 0.4, 1, 45, 0, 0, 0);
			this.visuals.image_ext(this.imgBuffer[12], +this.app.client.setWidth * 0.5, 65 + Math.cos(this.viewX / 15) * 5 + this.app.client.setHeight * 0.4, 1, 45, 1, 1, 1);
			this.visuals.image_ext(this.imgBuffer[1 - Math.round(Math.cos(this.viewX / 15) * 1)], this.app.client.setWidth * 0.5, 5 + Math.cos(this.viewX / 15) * 5 + this.app.client.setHeight * 0.4, 1, 45, 0, 0, 0);
			this.visuals.image_ext(this.imgBuffer[7], this.app.client.setWidth * 0.65, 5 + Math.cos(this.viewX / 15) * 5 + this.app.client.setHeight * 0.48, 1, 0.5, 1);
			this.visuals.image_ext(this.imgBuffer[8], this.app.client.setWidth * 0.35, 5 + Math.cos(this.viewX / 15) * 5 + this.app.client.setHeight * 0.48, 1, 0.5, 1);
		...
		if (this.gover) {
			...
			if (!this.best)
				this.visuals.image_ext(this.imgBuffer[11], this.app.client.setWidth / 3.1, this.app.client.setHeight * 0.55, 3, 0.2, 1);
			else
				this.visuals.image_ext(this.imgBuffer[10], this.app.client.setWidth / 3.1, this.app.client.setHeight * 0.55, 3, 0.8, 1);
			...
			this.visuals.image_button(this.imgBuffer[13], this.app.client.setWidth * 0.5, this.app.client.setHeight * 0.73, 1, function () {
				App.client.update.state.set(App.client._Main, true);
			}, this.imgBuffer[4], 1, 1, this.buttonfade, 1);
			// this.visuals.image_button(this.imgBuffer[14], this.app.client.setWidth * 0.7, this.app.client.setHeight * 0.73, 1, function () {
			// 	FB.ui(
			// 		{
			// 			method: 'feed',
			...
			// 		}
			// 	);
			// }, this.imgBuffer[3], 1, 1, this.buttonfade, 1);
